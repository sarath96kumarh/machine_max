...
def generate_data(t, state):
    if state == "ACTIVE":
        rms_vibration = np.clip(
            1250 + np.random.uniform(-750, 750, len(t)) + generate_noise(1250, 0.25),
            1000,
            2000,
        )  # 25% noise
        speed = np.clip(
            20 + np.random.uniform(-20, 20, len(t)) + generate_noise(20, 0.25), 10, 40
        )  # 25% noise
        voltage = np.clip(
            12500
            + np.random.uniform(-1500, 1500, len(t))
            + generate_noise(12500, 0.25),
            13000,
            14000,
        )  # 25% noise
    elif state == "IDLE":
        ...
        speed = np.clip(
            10 + np.random.uniform(-10, 10, len(t)) + generate_noise(10, 0.25), 0, 5
        )  # 25% noise
        voltage = np.clip(
            12500
            + np.random.uniform(-1500, 1500, len(t))
...
def simulate_data():
    ...
    while current_time < duration:
        ...
        if state == "ACTIVE":
            ...
            state_duration = np.random.randint(
                5 * 60, 20 * 60
        ...
        if current_time + state_duration < duration:
            ...
            rms_vibration = smooth_transition(
                rms_vibration,
                transition_start,
                transition_end,
                rms_vibration[transition_start],
                np.random.uniform(
                    500
                    if next_state == "ACTIVE"
                    else 500 if next_state == "IDLE" else 500
                ),
            )

            # Smooth Speed
            speed = smooth_transition(
                speed,
                transition_start,
                transition_end,
                speed[transition_start],
                np.random.uniform(
                    20 if next_state == "ACTIVE" else 10 if next_state == "IDLE" else 0
                ),
            )

            # Smooth Voltage
            voltage = smooth_transition(
                voltage,
                transition_start,
                transition_end,
                voltage[transition_start],
                np.random.uniform(
                    12500
                    if next_state == "ACTIVE"
                    else 12500 if next_state == "IDLE" else 12500
                ),
            )
    ...
    def add_feature_engineered_columns(df, window_size=60):
        ...
        df["State_Duration"] = df.groupby(
            (df["State"] != df["State"].shift()).cumsum()
        ).cumcount()

        # Rolling Statistics for Speed
        df["Speed_Rolling_Mean"] = (
            df["Speed"].rolling(window=window_size, min_periods=1).mean()
        )
        df["Speed_Rolling_Std"] = (
            df["Speed"].rolling(window=window_size, min_periods=1).std()
        )
